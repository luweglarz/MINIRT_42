...
/*   Updated: 2021/02/17 17:17:23 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		is_intersection(t_scene scene, t_vector ray_pos, t_vector light_dir)
{
	t_ray	ray;
...
t_frgb	compute_light(t_vector ray_pos, t_vector normal, t_scene *scene)
{
	...
	while (lights->next)
	{
		...
		if (is_intersection(*scene, ray_pos, light_dir))
			break;
